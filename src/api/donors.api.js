...
export const donorRegister = async (
  body
) =>
  httpClient.post(`/donor-register`, body);

export const getDonors = async () =>
  await httpClient.get(`/donors/list`);

export const getDonorsByCampaign = async (campaignId) =>
  await httpClient.get(`/donors/campaign/list/${campaignId}`);

export const verifySubscription = async (emailAddress, hash) =>
  await httpClient.get(`/subscription/verify/${emailAddress}?v=${hash}`);

export const confirmPledge = async (emailAddress, hash, campaignRequestId) =>
  await httpClient.get(`/donor/confirm-pledge/${emailAddress}?v=${hash}&c=${campaignRequestId}`);
...
export const changePledge = async (emailAddress, hash, changeDetail) =>
  await httpClient.post(`/donor/change-pledge/${emailAddress}?v=${hash}`, {details: changeDetail});

export const getDonorById = async (donorId) =>
  await httpClient.get(`/donor/view/${donorId}`); 

export const deleteDonor = async (body) => 
  await httpClient.post(`/donor/delete`, body);
...
export const donorEmailUpdate = async (
  body
) =>
  httpClient.post(`/donors/email-update`, body);

export const donorPledgeUpdate = async (
  body
) =>
