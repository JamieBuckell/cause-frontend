...
const getters = {
  ...
  usersName: (state) => state.currentUser.authenticated ? state.currentUser.fullname : 'Unknown',
  usersEmail: (state) => state.currentUser.username ? state.currentUser.username : '',
...
const actions = {
  ...
  completeNewPasswordChallenge({ state, commit, dispatch }, authData) {
      state.cognitoUser.completeNewPasswordChallenge(authData.Password, state.attributes, {
        onSuccess: result => {
  ...
  async checkTokenExpiration({ state, commit, dispatch }) {
    ...
    if (timestamp - waitTime >= state.timestamps.lastAuthenticated) { 
      const AccessToken = new AmazonCognitoIdentity.CognitoAccessToken({AccessToken: state.tokens.accessToken});
      const IdToken = new AmazonCognitoIdentity.CognitoIdToken({IdToken: state.tokens.idToken});
      const RefreshToken = new AmazonCognitoIdentity.CognitoRefreshToken({RefreshToken: state.tokens.refreshToken});
      const sessionData = {
        IdToken: IdToken,
